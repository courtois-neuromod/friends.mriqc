...
#SBATCH --job-name=mriqc_study-friends_sub-04_ses-020.job
#SBATCH --output=./code/mriqc_study-friends_sub-04_ses-020.out
#SBATCH --error=./code/mriqc_study-friends_sub-04_ses-020.err
#SBATCH --time=8:00:00
...
datalad containers-run -m 'mriqc_sub-04/ses-020' -n containers/bids-mriqc --input sourcedata/friends/sub-04/ses-020/fmap/ --input sourcedata/friends/sub-04/ses-020/func/ --output . -- -w workdir/ --participant-label 04 --session-id 020 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-sub sourcedata/friends ./ participant 
mriqc_exitcode=$?
