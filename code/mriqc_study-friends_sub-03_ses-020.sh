...
#SBATCH --job-name=mriqc_study-friends_sub-03_ses-020.job
#SBATCH --output=./code/mriqc_study-friends_sub-03_ses-020.out
#SBATCH --error=./code/mriqc_study-friends_sub-03_ses-020.err
#SBATCH --time=8:00:00
...
datalad containers-run -m 'mriqc_sub-03/ses-020' -n containers/bids-mriqc --input sourcedata/friends/sub-03/ses-020/fmap/ --input sourcedata/friends/sub-03/ses-020/func/ --output . -- -w workdir/ --participant-label 03 --session-id 020 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-sub sourcedata/friends ./ participant 
mriqc_exitcode=$?
