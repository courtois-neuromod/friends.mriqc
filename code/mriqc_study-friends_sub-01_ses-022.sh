...
#SBATCH --job-name=mriqc_study-friends_sub-01_ses-022.job
#SBATCH --output=./code/mriqc_study-friends_sub-01_ses-022.out
#SBATCH --error=./code/mriqc_study-friends_sub-01_ses-022.err
#SBATCH --time=8:00:00
...
datalad containers-run -m 'mriqc_sub-01/ses-022' -n containers/bids-mriqc --input sourcedata/friends/sub-01/ses-022/fmap/ --input sourcedata/friends/sub-01/ses-022/func/ --output . -- -w workdir/ --participant-label 01 --session-id 022 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-sub sourcedata/friends ./ participant 
mriqc_exitcode=$?
