...
#SBATCH --job-name=mriqc_study-friends_sub-01_ses-023.job
#SBATCH --output=./code/mriqc_study-friends_sub-01_ses-023.out
#SBATCH --error=./code/mriqc_study-friends_sub-01_ses-023.err
#SBATCH --time=8:00:00
...
datalad containers-run -m 'mriqc_sub-01/ses-023' -n containers/bids-mriqc --input sourcedata/friends/sub-01/ses-023/fmap/ --input sourcedata/friends/sub-01/ses-023/func/ --output . -- -w workdir/ --participant-label 01 --session-id 023 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-sub sourcedata/friends ./ participant 
mriqc_exitcode=$?
